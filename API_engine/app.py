...
@app.get(
    "/asins",
    description="List all the available asins",
    tags=["Inventory"]
)
def _list_asins():
    # TODO Filters
...
@app.get(
    ...
    description="Get a single asin by its unique ID",
    responses=get_exception_responses(AsinInfoNotFoundException),
    tags=["Inventory"]
)
def _get_asin(asin_id: str):
    return InventoryRepository.get(asin_id)


@app.post(
    "/asins",
    description="Create a new asin",
    response_model=AsinInfoRead,
    status_code=statuscode.HTTP_201_CREATED,
...
@app.patch(
    ...
    status_code=statuscode.HTTP_204_NO_CONTENT,
    responses=get_exception_responses(AsinInfoNotFoundException, AsinInfoAlreadyExistsException),
    tags=["Inventory"]
)
def _update_asin(asin_id: str, update: AsinInfoUpdate):
    InventoryRepository.update(asin_id, update)


@app.delete(
    "/asins/{asin_id}",
    description="Delete a single asin by its unique ID",
    status_code=statuscode.HTTP_204_NO_CONTENT,
    responses=get_exception_responses(AsinInfoNotFoundException),
    tags=["Inventory"]
)
def _delete_asin(asin_id: str):
    InventoryRepository.delete(asin_id)
